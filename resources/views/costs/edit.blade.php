<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <!-- Cost Form Card -->
            <x-card title="Edit Cost Information" footer='codeyad.com'>
                <x-form.container action="{{ route('costs.update', $cost) }}" method="POST" id="costForm"
                    class="space-y-6">
                    @csrf
                    @method('PATCH')
                    ...
                    <x-form.input name="name" label="Cost Name" placeholder="Enter cost name" required
                        maxlength="100" oninput="validateName(this)" :value="$cost->name" />
                    ...
                    <x-form.select name="category_id" label="Category" placeholder="Select a category" required oninput="validateCategory(this)">
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ $cost->category_id == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </x-form.select>
                    ...
                    <x-form.input name="amount" label="Amount" type="number" placeholder="Enter amount" required
                        oninput="validateAmount(this)" :value="$cost->amount" />
                    ...
                    <x-forms.datepicker name="paid_at" label="Paid At" required oninput="validateDate(this)" 
                        :value="$cost->paid_at->format('Y-m-d')" />
